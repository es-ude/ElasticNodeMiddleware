...
void readValue(uint32_t *destination)
{
    ...
    debugWriteStringLength((uint8_t *)destination, sizeof(uint32_t));
...
void configurationUartFlash(void) {
    ...
    uint16_t numBlocks4K = ceil((float)(configSize) / 0x1000)+1;
...
//        while(read_a_block_of_data(buffer)!=blockSize);
        ...
        writeDataFlash(currentAddress, buffer, blockSize, 0);
